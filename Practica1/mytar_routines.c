 ...
 */
int copynFile(FILE * origin, FILE * destination, int nBytes, uint16_t * crc)
{
	int nCopy = 0;
	int c;
 ...
 */
stHeaderEntry* readHeader(FILE * tarFile, int *nFiles, uint16_t *crc)
{
	int i,j;
	...
	for (i = 0; i < *nFiles; i++) {
		...
		fread(&header[i].crc, sizeof(uint16_t), 1, tarFile);
		//crc = &header[i].crc; 
	}
...
int createTar(int nFiles, char *fileNames[], char tarName[])
{
	...
	//uint16_t *crc = malloc(sizeof(uint16_t));
	uint16_t crc;
	...
	for (i = 0; i < nFiles; i++) {
		...
		headersize += namesize + sizeof (header->size)+sizeof (uint16_t);
	}
	...
	for (i = 0; i < nFiles; i++) {
		...
		header[i].size = copynFile(inputFile, tarFile, INT_MAX, &crc);
		header[i].crc = crc;
		fclose(inputFile);
	}
	...
	for (i = 0; i < nFiles; i++) {
		...
		fwrite(&header[i].crc, sizeof (uint16_t), 1, tarFile);
	}
...
int extractTar(char tarName[])
{
	...
	header = readHeader(tarFile, nFiles, &crc);//leemos la cabecera
	...
	for (i = 0; i < *nFiles; i++){
		...
		fwrite(&crc, header[i].crc, 1, outputFile);
		printf("Fetcher16 of : 0x%.4X\n",crc);
